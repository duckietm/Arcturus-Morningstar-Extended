...
/**
 * Passes when the resolved user's credits compare to {@code amount} as the operator says - the
 * positive half of {@link WiredConditionHabboLacksCredits}, which was the only half the emulator had.
 * A furni named {@code wf_cnd_habbo_has_credits} was on sale pointing at the negative class, so a
 * builder who asked for "the user has credits" got a box that passed only when they did not.
 *
 * <p>It keeps {@link WiredConditionTeamGameBase}'s storage - amount, user source and quantifier are the
 * settings it needs - and answers {@link WiredConditionType#USER_AMOUNT}. The operator is
 * {@link WiredComparison}'s six-code table; a row saved before it was honoured reads as {@code >=},
 * the one way this box used to compare.</p>
 */
public class WiredConditionHabboHasCredits extends WiredConditionTeamGameBase {
    ...
    public WiredConditionHabboHasCredits(
    ...
    private boolean matchesUser(Room room, RoomUnit roomUnit) {
        ...
        return WiredComparison.compare(habbo.getHabboInfo().getCredits(), this.amount, this.comparison);
    }
