...
/**
 * Passes when the resolved user holds at least {@code amount} duckets - the positive half of
 * {@link WiredConditionHabboLacksDuckets}, which was the only half the emulator had. A furni named
 * {@code wf_cnd_habbo_has_duckets} was on sale pointing at the negative class, so a builder who asked
 * for "the user has duckets" got a box that passed only when they did not.
 *
 * <p>It keeps {@link WiredConditionTeamGameBase}'s storage - amount, user source and quantifier are the
 * settings it needs - and answers {@link WiredConditionType#USER_AMOUNT}, whose dialog leaves out the
 * team colour and the comparison operator. Like its twin it compares one way: {@code >= amount}.</p>
 */
