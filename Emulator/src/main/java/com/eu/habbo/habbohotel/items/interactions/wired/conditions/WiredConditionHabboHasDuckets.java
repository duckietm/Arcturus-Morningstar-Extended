...
/**
 * Passes when the resolved user's duckets compare to {@code amount} as the operator says - the
 * positive half of {@link WiredConditionHabboLacksDuckets}, which was the only half the emulator had.
 * A furni named {@code wf_cnd_habbo_has_duckets} was on sale pointing at the negative class, so a
 * builder who asked for "the user has duckets" got a box that passed only when they did not.
 *
 ...
 */
public class WiredConditionHabboHasDuckets extends WiredConditionTeamGameBase {
    ...
    public WiredConditionHabboHasDuckets(
